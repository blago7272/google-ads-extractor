{#
    Latest keyword dimension state per account and transfer source.

    Same shape as stg_ad_dimension_latest: the raw `p_ads_Keyword_*` transfer tables
    are DAILY FULL SNAPSHOTS (~788M rows / ~200 GB for ~2.6M distinct keywords), so
    reading the full wildcard on every run rescans all history to pick the newest row
    per keyword — ~22 GB per execution and growing daily.

    Accumulating dimension: merge only the recent snapshot partitions onto the stored
    state. Keywords that stop appearing keep their last-known attributes, so historical
...
{{
    config(
        ...
        unique_key=['transfer_source', 'account_id', 'campaign_id', 'ad_group_id', 'keyword_id'],
